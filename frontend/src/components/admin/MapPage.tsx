...
export const MapPage = () => {
    ...
    const tm = t as unknown as (key: string, opts?: Record<string, unknown>) => string;
        ...
        <div className="space-y-6 h-[calc(100vh-140px)] flex flex-col">
            <div className="flex items-center justify-between shrink-0">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm mt-1">{tm('subtitle')}</p>
                </div>
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-4 bg-muted/40 p-2 rounded-lg border border-border">
                        <div className="flex items-center gap-2 px-3 border-r border-border">
                            <Home className="w-4 h-4 text-primary" />
                            <span className="text-sm font-semibold">{tm('stats.properties', { count: properties.length })}</span>
                        </div>
                        <div className="flex items-center gap-2 px-3">
                            <MapPin className="w-4 h-4 text-primary" />
                            <span className="text-sm font-semibold text-muted-foreground">{tm('stats.city')}</span>
                        </div>
                    </div>

                    {/* Pin Renk Sistemi Legend */}
                    <div className="bg-muted/40 p-3 rounded-lg border border-border">
                        <h3 className="text-xs font-semibold mb-2 text-muted-foreground uppercase tracking-wide">{tm('pinLegend.title')}</h3>
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#3b82f6' }}></div>
                                <span>{tm('pinLegend.blue')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#10b981' }}></div>
                                <span>{tm('pinLegend.green')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#f97316' }}></div>
                                <span>{tm('pinLegend.orange')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#ec4899' }}></div>
                                <span>{tm('pinLegend.red')}</span>
                            </div>
                            <div className="flex items-center gap-2 col-span-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#6b7280' }}></div>
                                <span>{tm('pinLegend.gray')}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    {!isEditing ? (
                        <>
                            ...
                            <div className="px-10 py-5 bg-muted/30 border-t border-border flex justify-end gap-3 shrink-0">
                                <Button variant="outline" onClick={handleClose}>{tm('dialogs.close')}</Button>
                                <Button onClick={() => setIsEditing(true)}>
                                    <Pencil className="w-4 h-4 mr-2" /> {tm('dialogs.edit')}
                                </Button>
                            </div>
                        </>
                    ) : (
                        <>
                            <div className="px-8 py-5 border-b border-border bg-card/50 z-10 shrink-0">
                                <DialogHeader>
                                    <DialogTitle className="text-2xl font-bold tracking-tight text-foreground">{tm('dialogs.editTitle')}</DialogTitle>
                                    <DialogDescription className="text-sm text-muted-foreground mt-1.5">
                                        {tm('dialogs.editDescription')}
                                    </DialogDescription>
                                </DialogHeader>
                            </div>
                            <div className="px-8 py-6 overflow-y-auto flex-1">
